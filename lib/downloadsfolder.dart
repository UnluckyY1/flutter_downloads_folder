...
///
///
/// - On **Android**, this method uses the platform-specific channel to invoke a native method ('getExternalStoragePublicDirectory')
/// and retrieves the path to the external storage downloads directory based on the specified folder type [_androidDownloadsFolderType].
///
...
///
/// @remarks
/// This method checks the Android SDK version and utilizes a workaround to avoid using `MANAGE_EXTERNAL_STORAGE` on Android 29 and higher.
/// For devices with Android API 29 and higher, the method uses the platform-specific channel to invoke a native method ('saveFileUsingMediaStore')
/// and saves the file using `MediaStore` to bypass the restriction.
///
/// On devices with Android versions below 29, the method gets the path to the download folder and copies the file using the `copyTo` method.
/// If the destination file already exists, a unique name is generated by appending a suffix in the form of '_(copyNumber)' to the file name.
...
///
/// return A [Future] that completes with a [bool] value indicating whether the operation to open the download folder was successful or not.
///         Returns `true` if successful, otherwise returns `false`.
///
///
/// - On **Android** and **iOS**, this method uses the platform-specific channel to invoke a native method ('openDownloadFolder')
/// to open the download folder directly, providing a seamless user experience.
//
//
/// - On other platforms (e.g., **macOS**, **Windows**), the method retrieves the path to the download folder using the [getDownloadFolderPath] method,
/// and attempts to launch the file explorer or file manager to open the specified folder using the [launchUrl] function.
///
